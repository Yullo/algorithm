...
public class Solution300 {
     ...
     * 时间复制度 O (n log n) 的解法：
     *
     * 另外创建一个数组A用以保存最长上升子序列 （不一定是最终子序列，只是长度相同） ， 一开始为空。
     * 遍历数组，如果该值大于子序列的末尾， 则将该值加入达到子序列之中
     * 如果该值小于子序列的开头，则替换子序列的开头为自己
     * 如果该值介于子序列开头和结尾之间， 使用二分查找法，找到子序列中第一个大于该值的数，将其替换为自己
     * 遍历完成后，可以得到最长上升子序列长度。 这个过程只遍历一遍数组, 二分查找子序列，时间复杂度 n log n
     * 如果需要输出子序列，再使用一个数组B保存原数组放在A的位置，知道长度后，从后往前遍历B数组，从原数组获取元素即可
     */
